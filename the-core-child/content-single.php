...
<article id="post-<?php the_ID(); ?>" <?php post_class( "post post-details" ); ?> itemscope="itemscope" itemtype="https://schema.org/BlogPosting" itemprop="blogPost">
	<div class="fw-col-inner">
		...
		<?php if ( $the_core_post_options['image'] && $the_core_post_options['featured_image'] != 'no') : ?>
			<?php 
				// ajouté par CMA pour contrôler la position de la featured image via un CSS
                $featured_image_position_class = "";
                if( get_field('featured_image_position') == 'right' ) {
                    $featured_image_position_class = "cma-pgla_featured-image-position-right";
                } elseif (get_field('featured_image_position') == 'left' ) {
                    $featured_image_position_class = "cma-pgla_featured-image-position-left";
				}
				// la variable est ajoutée dans le div suivant
				// top est la valur par défaut du thème, donc on ne fait rien dans ce cas là
			?>
			<div class="fw-post-image fw-block-image-parent <?php echo esc_attr($the_core_post_options['frame']); ?> <?php echo $featured_image_position_class; ?>">
